//класс для того чтобы сделать QLabel нажимаемым
//по своей сути - объект, который отслеживает события, происходящие с привязанным виджетом
//если это событие - нажатие левой кнопкой мыши - то событие регистрируется и вызывается сигнал о нажатии
//в самом сигнале передается место в зале, которое получается из названия лейбла
